...
def simulacao_completa(prob_cliente_normal_recrutar = 0.03,
    ...
    bonus_construtor_map = {
        "Team Builder 10k": 0.0,
        "Team Builder 20k": 2000.0,
        "Team Builder 30k": 4500.0,
        "Team Leader 50k": 4500.0,
        "Team Leader 100k": 4500.0,
        "Team Leader 150k": 4500.0,
        "Legacy Maker 200k": 4500.0,
        "Legacy Maker 400k": 4500.0,
        "Legacy Maker 600k": 4500.0,
        "Legend 1M": 4500.0,
        "Legend 3M": 4500.0,
        "Legend 5M": 4500.0
    }
    ...
    time.sleep(1)
    categorias_team_builder_30k_ou_superior = {"Team Builder 30k", "Team Leader 50k", "Team Leader 100k", "Team Leader 150k", 
    ...
    def calcula_bonus(pessoa, filhos, vtp, categoria):
        ...
        # Afiliado
        if categoria == "Influencer 1k":
            bonus += 0.10 * sum(vtp.get(f, 0) for f in filhos_)

        # Influencer
        elif categoria in ["Influencer 5k"]:
        ...
        # Team Builder 20k
        elif categoria == "Team Builder 20k":
            bonus += 0.15 * sum(vtp.get(f, 0) for f in filhos_)
            bonus += 0.05 * sum(vtp.get(f, 0) for f in netos_)
            bonus += 0.05 * sum(vtp.get(f, 0) for f in bisnetos_)
        
        # Team Builder 30k ou mais
        elif categoria in categorias_team_builder_30k_ou_superior:
            bonus += 0.15 * sum(vtp.get(f, 0) for f in filhos_)
            bonus += 0.075 * sum(vtp.get(f, 0) for f in netos_)
            bonus += 0.075 * sum(vtp.get(f, 0) for f in bisnetos_)
    ...
    def calcular_bonus_lideranca(df_mes: pd.DataFrame, pat_map_global: dict) -> pd.DataFrame:
        ...
        # Parte nova pois cada geracao recebe pontuacao diferente
        def pct_por_geracao(gen: int) -> float:
            if gen <= 2:
                return 0.06
            elif gen <= 5:
                return 0.05
        ...
        # Para o novo cálculo apenas essa função foi alterada
        def coletar_geracao(filhos_diretos, n_geracoes):
            vtp_por_gen = defaultdict(float)
            for filho in filhos_diretos:
                ...
                while queue:
                    ...
                    if gen > n_geracoes:
                        continue
                    vtp_por_gen[gen] += vtp_map.get(node, 0)
            ...
            return sum(pct_por_geracao(gen) * vtp for gen, vtp in vtp_por_gen.items())
        ...
        for pid in df["idPessoa"]:
            ...
            _, n_geracoes = bonus_lideranca_map[cat_map[pid]]
            ...
            bonus = coletar_geracao(filhos_diretos, n_geracoes)
            df.loc[df["idPessoa"] == pid, "bonus_lideranca"] = bonus
